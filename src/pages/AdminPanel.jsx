...
export default function AdminPanel() {
  ...
  // Mövcud useEffect-in yanına əlavə et:
  useEffect(() => {
    if (
      !authLoading &&
      profile &&
      ...
      navigate("/"); // icazəsiz user ana səhifəyə
    }
  ...
  async function fetchStats() {
    ...
    setStats({ ...counts, articles: articleCount || 0 });
  }

  // ── Statistika UI — Header-in altına əlavə et (yalnız admin görür) ──
      ...
      {profile?.role === "admin" && stats && (
        <div className="max-w-5xl mx-auto px-6 pt-6">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {[
              { label: "Uşaq", count: stats["Uşaq"], emoji: "🌱" },
              { label: "Tələbə", count: stats["Tələbə"], emoji: "🎓" },
              { label: "Valideyn", count: stats["Valideyn"], emoji: "👨‍👩‍👧" },
              {
                label: "Mütəxəssis",
                count:
                  (stats["Psixoloqlar"] || 0) +
              ...
              { label: "Məqalə", count: stats["articles"], emoji: "📝" },
            ].map((s) => (
              <div
                key={s.label}
