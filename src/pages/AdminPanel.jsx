...
import {
  faChild, faUserGraduate, faUsers,
  faBriefcase, faNewspaper,
  faImage, faXmark, faSpinner,
...
export default function AdminPanel() {
  ...
  /* ── auth yoxlamaları ── */
  useEffect(() => { if (user && profile?.role === "admin") fetchStats(); }, [user, profile]);
  useEffect(() => { if (!authLoading && !user) navigate("/login"); },      [user, authLoading, navigate]);
  useEffect(() => { if (user) fetchMyArticles(); },                        [user]);
  useEffect(() => {
    if (!authLoading && profile &&
      !["admin","Psixoloqlar","Loqopedlər","Pedaqoqlar","EQ","IQ","Mentor"].includes(profile.role))
      navigate("/");
  ...
  async function fetchStats() {
    ...
    const counts = { "Uşaq":0,"Tələbə":0,"Valideyn":0,"Psixoloqlar":0,"Loqopedlər":0,"Pedaqoqlar":0,"EQ":0,"IQ":0,"Mentor":0 };
    ...
    setStats({
      ...
      mutexessis: (counts["Psixoloqlar"]||0)+(counts["Loqopedlər"]||0)+
                  (counts["Pedaqoqlar"]||0)+(counts["EQ"]||0)+(counts["IQ"]||0)+(counts["Mentor"]||0),
    });
  }
  ...
  function handleFileSelect(e) {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      setImgError("Yalnız JPG, PNG, WebP və GIF formatları qəbul edilir");
      return;
    }
    // Ölçü yoxlaması
  ...
  /* ── form ── */
  const allowedAgeGroups   = profile ? (ROLE_AGE_GROUPS[profile.role]  || ROLE_AGE_GROUPS["admin"])  : ROLE_AGE_GROUPS["admin"];
  const allowedCategories  = profile ? (ROLE_CATEGORIES[profile.role]  || ROLE_CATEGORIES["admin"]) : ROLE_CATEGORIES["admin"];
  const availableCategories = form.age_group
    ? (ageGroups.find(g=>g.id===form.age_group)?.categories||[]).filter(c=>allowedCategories.includes(c))
    : [];
  ...
  async function handleSave() {
    ...
    const payload = { ...form, author_id: user.id, image_url: imageUrl || form.image_url || null };
    let error;
    if (editId) {
  ...
  if (authLoading || !profile) {
    return <div className="min-h-screen flex items-center justify-center text-gray-400">Yüklənir...</div>;
  }
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => { setShowForm(true); setEditId(null); setForm(EMPTY_FORM); setImgFile(null); setImgPreview(null); }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">
                  Şəkil <span className="text-gray-400 normal-case font-normal">(JPG, PNG, WebP, GIF · max {MAX_SIZE_MB}MB)</span>
                </label>
                ...
                {imgPreview ? (
                  ...
                  >
                    <FontAwesomeIcon icon={faImage} className="text-3xl text-gray-300 group-hover:text-emerald-400 mb-2 transition-colors" />
                    <p className="text-sm text-gray-400 group-hover:text-emerald-500 transition-colors">
                      Şəkil seçmək üçün klikləyin
                    </p>
                    <p className="text-xs text-gray-300 mt-1">JPG, PNG, WebP, GIF · max {MAX_SIZE_MB}MB</p>
                  </label>
                )}
