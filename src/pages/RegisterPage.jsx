...
const BASE_ROLES = [
  { value: "Uşaq",        label: "Uşaq",       emoji: "🌱", desc: "6–17 yaş" },
  { value: "Tələbə",      label: "Tələbə",      emoji: "🎓", desc: "Universitet" },
  { value: "Valideyn",    label: "Valideyn",    emoji: "👨‍👩‍👧", desc: "Övladım var" },
  { value: "Mütəxəssis",  label: "Mütəxəssis",  emoji: "💼", desc: "Peşəkar" },
];

const SPECIALIST_ROLES = [
  { value: "Psixoloqlar", label: "Psixoloq",      emoji: "🧠" },
  { value: "Loqopedlər",  label: "Loqoped",        emoji: "🗣" },
  { value: "Pedaqoqlar",  label: "Pedaqoq",        emoji: "👨‍🏫" },
  { value: "EQ",          label: "EQ Mütəxəssisi", emoji: "❤️" },
  { value: "IQ",          label: "IQ Mütəxəssisi", emoji: "🧩" },
...
export default function RegisterPage() {
  ...
  const finalRole = form.baseRole === "Mütəxəssis" ? form.specRole : form.baseRole;
  ...
  async function handleSubmit(e) {
    ...
    if (form.baseRole === "Mütəxəssis" && !form.specRole) {
      setError("Mütəxəssis növünü seçin"); return;
    }
    if (form.password !== form.password2) {
      setError("Şifrələr uyğun gəlmir"); return;
    }
    if (form.password.length < 6) {
      setError("Şifrə ən az 6 simvol olmalıdır"); return;
    }
    setLoading(true);
    try {
      ...
      const { error: profileError } = await supabase.from("profiles").insert({
        id: userId, full_name: form.fullName,
        role: finalRole, avatar_url: avatarUrl,
      });
      ...
      <div className="min-h-screen pt-20 bg-gradient-to-br from-emerald-50 to-teal-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-3xl shadow-xl p-10 text-center max-w-sm w-full">
          ...
          <p className="text-gray-500 text-sm mb-2">
            <span className="font-semibold text-emerald-600">{form.email}</span> ünvanına təsdiq linki göndərildi.
          </p>
          <p className="text-gray-400 text-xs mb-6">Linki kliklədikdən sonra daxil ola bilərsiniz.</p>
          <Link to="/login" className="inline-block bg-emerald-500 hover:bg-emerald-600 text-white font-semibold px-6 py-2.5 rounded-xl text-sm transition-colors">
            Daxil ol →
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen pt-20 bg-gradient-to-br from-emerald-50 via-white to-teal-50 flex items-center justify-center px-4 py-6">
      ...
      <div className="relative w-full max-w-4xl">
        <div className="bg-white rounded-3xl shadow-2xl border border-emerald-100/50 overflow-hidden">
          <div className="grid grid-cols-1 lg:grid-cols-5">
            ...
            <div className="lg:col-span-2 bg-gradient-to-b from-emerald-500 to-teal-600 p-8 flex flex-col justify-between text-white">
              <div>
                ...
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <span className="font-bold text-sm opacity-90">İnkişaf Akademiyası</span>
                </div>
                ...
                <label className="cursor-pointer block">
                  <div className="w-20 h-20 rounded-2xl border-2 border-dashed border-white/40 hover:border-white/80 transition-colors overflow-hidden flex items-center justify-center bg-white/10 mb-2">
                    {preview
                      ...
                      : <div className="text-center"><div className="text-2xl">📷</div><div className="text-xs text-white/60 mt-1">Şəkil</div></div>
                    }
                  </div>
                  <input type="file" accept="image/*" onChange={handleAvatar} className="hidden" />
                </label>
                <p className="text-xs text-emerald-200">Profil şəkli (istəyə bağlı)</p>
              </div>

              {/* Alt hissə */}
              <div className="mt-8 pt-6 border-t border-white/20">
                <p className="text-emerald-100 text-xs mb-1">Artıq hesabınız var?</p>
                <Link to="/login" className="text-white font-semibold text-sm hover:opacity-80 transition-opacity">
                  Daxil olun →
                </Link>
              </div>
            </div>
            ...
            {/* ── Sağ panel (3/5) ── */}
            <div className="lg:col-span-3 p-8 overflow-y-auto" style={{ maxHeight: "calc(100vh - 80px)" }}>
              <h2 className="text-xl font-bold text-gray-900 mb-1">Hesab yaradın</h2>
              <p className="text-gray-500 text-sm mb-6">Bütün sahələri doldurun</p>

              <form onSubmit={handleSubmit} className="space-y-4">

                {/* Kim olduğunuzu seçin */}
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">Siz kisiniz? *</label>
                  <div className="grid grid-cols-2 gap-2">
                    {BASE_ROLES.map((r) => (
                      <button
                        key={r.value}
                        type="button"
                        onClick={() => { handleChange("baseRole", r.value); handleChange("specRole", ""); }}
                ...
                {form.baseRole === "Mütəxəssis" && (
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">Sahəniz *</label>
                    <div className="grid grid-cols-2 gap-2">
                ...
                {/* Şifrə + Şifrə təkrar — yan-yana */}
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Şifrə *</label>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Təkrar *</label>
                    <div className="relative">
                      <input
